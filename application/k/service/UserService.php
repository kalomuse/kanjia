...
class UserService extends Model
{
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            $map['subscribe'] = isset($data['subscribe'])?$data['subscribe']:$this->is_subscribe();
    ...
    private function is_subscribe() {
        ...
        $area_map = C('website_wx');
        if($area_map[$url]['secret']) {
            $name = $area_map[$url]['alias'];
            $jssdk = new Jssdk($area_map[$url]['appid'], $area_map[$url]['secret']);
            $userinfo = $jssdk->getUserInfoForOpenID(session('openid'), $name);
     ...
     * @return array
     */
    public function reg($username, $password, $area='', $alias=''){
        $is_validated = 0 ;
